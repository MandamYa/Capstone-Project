...
function initPanduanSlides() {

    slides = Array.from(
        document.querySelectorAll(
            '#panduan-slide-container .panduan-slide'
        )
    );
...
function renderCurrentSlide() {
    ...
    // Pastikan currentSlide tidak keluar batas
    if (currentSlide < 0) {
        currentSlide = 0;
    }
...
function updateSlide(direction) {
    ...
    // Jika berada di slide pertama
    if (newSlide < 0) {
        return;
    }


    // Jika sudah di slide terakhir
    if (newSlide >= totalSlides) {
        return;
    }


    // Simpan posisi slide baru
...
function openPanduanModal() {
    ...
    const panduanModal =
        document.getElementById('panduan-modal');


    if (!panduanModal) {

        console.error(
            'Modal panduan tidak ditemukan.'
        );
    ...
    // Jika slide belum diinisialisasi
    if (slides.length === 0) {
        initPanduanSlides();
    }
    ...
    // Tampilkan modal
    panduanModal.style.display = 'block';
